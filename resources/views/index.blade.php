  ...
  <div class="ms-auto d-flex gap-2 align-items-center">
    ...
    <div class="dropdown">
      <button class="btn btn-secondary dropdown-toggle" type="button" id="cityDropdown" data-bs-toggle="dropdown" aria-expanded="false">
        {{ request('city') ? ucfirst(request('city')) : 'All Cities' }}
      </button>
      <ul class="dropdown-menu" aria-labelledby="cityDropdown" id="cityMenu">
        <li><a class="dropdown-item city-item" href="{{ route('projects.index') }}">All Cities</a></li>
        @foreach(\App\Models\Project::select('city')->distinct()->get() as $p)
         <li><a class="dropdown-item city-item" href="?city={{ $p->city }}" data-city="{{ $p->city }}">{{ ucfirst($p->city) }}</a></li>
        @endforeach
      </ul>
    </div>
    
    <input type="hidden" id="cityFilter" value="{{ request('city', 'all') }}">
    <a href="{{ route('cities.index') }}" class="btn btn-primary">المحافظات</a>
    ...
    <form action="/" method="GET" class="d-flex align-items-center m-0">
      <select name="sort" class="form-select w-auto" onchange="this.form.submit()" style="height: 38px;">
        ...
        <option value="alphabetical"{{ request('sort') == 'alphabetical' ? 'selected' : '' }}>أبجديا(A-Z)</option>
      </select>
    </form> 
...
<div class="container-fluid mt-4">
  ...
  <div class="row mb-4">
    <div class="col-12">
      <div class="card p-4 shadow-sm border-0">
        ...
        <form action="{{ route('projects.store') }}" method="POST" enctype="multipart/form-data" class="row g-3" style="direction: rtl;">
          @csrf
      ...
      <div class="forecast-box shadow mb-4">
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-6 text-center border-start border-secondary">
                ...
                <div id="predictedPrice" class="prediction-value">{{ number_format($averagePrice, 0) }} ج.م</div>
  ...
  <div class="row g-3 mb-4 text-center" id="overview">
    <div class="col-md-3 col-6"><div class="card p-3 shadow-sm border-0"><h6>Avg Price</h6><h3 id="avgPrice" class="text-primary fw-bold">{{ number_format($averagePrice, 0) }} ج</h3></div></div>
    <div class="col-md-3 col-6"><div class="card p-3 shadow-sm border-0"><h6>Total Projects</h6><h3 id="totalTx" class="text-success fw-bold">{{ $totalProjects }}</h3></div></div>
        ...
        @foreach($properties as $property)
          <tr>
            <td>
              {{-- عرض الصور --}}
              @if($property->image)
                <img src="{{ asset($property->image) }}" alt="Project Image" style="width: 60px; height: 45px; object-fit: cover; border-radius: 5px; border: 1px solid #ddd;">
              @else
            ...
            <td><strong>{{ $property->name }}</strong><br><small class="text-muted">{{ ucfirst($property->city) }}</small></td>
            <td class="text-primary fw-bold">{{ number_format($property->price) }} ج</td>
            <td>
              <form action="{{ route('projects.destroy', $property->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟')">
                  @csrf @method('DELETE')
...
{{-- كود الـ JavaScript الخاص بالتنبؤ التفاعلي --}}
<script>
